    ...
    public function parse($data)
    {
        ...
        if (! empty($sheetData)) {
          $headers = [];
            ...
            if (stripos($data['fileName'], '30Days') > 0) {
                $requiredHeaders = ['employeeListId', 'employeeNumber', 'yearId', 'sgk_day', 'netucret', 'yol', 'yemek'];
            } else {
                $requiredHeaders = ['employeeListId', 'employeeNumber', 'yearId', 'netucret', 'netucret_day', 'yol', 'yol_day', 'yemek', 'yemek_day'];
            }
            unset($sheetData[0]);
